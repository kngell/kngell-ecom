...
abstract class AbstractCheckoutformSteps
{
    use DisplayTraits;
    ...
    {
        $btnGroup = $this->paths->offsetGet('buttonGroupPath');
        $this->isFileexists($btnGroup);
        $btnGroup = file_get_contents($btnGroup);
        $btnGroup = str_replace('{{buttonsLeft}}', $this->btns->buttonsGroup('prev'), $btnGroup);
        $btnGroup = str_replace('{{buttonsRight}}', $this->btns->buttonsGroup('next'), $btnGroup);
        return $btnGroup;
    }
